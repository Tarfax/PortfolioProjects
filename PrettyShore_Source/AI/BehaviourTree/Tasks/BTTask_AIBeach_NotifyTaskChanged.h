...
#include "BTTask_AIBeach_NotifyTaskChanged.generated.h"

UCLASS()
class UBTTask_AIBeach_NotifyTaskChanged : public UBTTaskNode {
public:
	GENERATED_BODY()
	UBTTask_AIBeach_NotifyTaskChanged();

	void OnGameplayTaskActivated(UGameplayTask& Task) override;
	EBTNodeResult::Type ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory) override;
	void InitializeFromAsset(UBehaviorTree& Asset) override;
	void TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds) override;
	bool SetAIVisitorState(UBehaviorTreeComponent& OwnerComp) const;
	bool SetAILifeGuardState(UBehaviorTreeComponent& OwnerComp) const;
